  ...
  async function handleWhatsappShare(postId) {
    ...
    try {
      ...
      if (res.success) {
        ...
        addToast('Successfully formatted WhatsApp broadcast message!', 'success');
        setSuccessMsg('WhatsApp broadcast message ready and opened!');
      } else {
        addToast(res.message || 'WhatsApp sharing failed.', 'error');
        setError(res.message || 'WhatsApp sharing failed.');
      }
    } catch (err) {
      addToast(err.message || 'Failed to share post to WhatsApp', 'error');
      setError(err.message || 'Failed to share post to WhatsApp');
    } finally {
      setTgLoadingId(null);
    }
  }
