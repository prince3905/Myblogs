# Next Features

## 1. Push to GitHub
- Commit all changes and push to remote repository

## 2. Monetization
- Google AdSense ad slots (in-content, sidebar, after-post)
- Affiliate link support with configurable tracking
- Sponsored post badge/flag

## 3. Full-Text Search
- `GET /api/posts/search?q=<query>` endpoint with text index on title + content
- Search page (`/search`) with results display
- Debounced search bar in header

## 4. SEO Enhancements
- Dynamic `<title>` and `<meta>` tags per page via `react-helmet-async`
- Sitemap auto-generation (`/sitemap.xml`)
- `robots.txt` endpoint
- Open Graph / Twitter card meta tags for social previews

## 5. Email Notifications
- Notify admin on new comment submission
- Notify admin on new contact form message
- (Optional) Welcome email to newsletter subscribers

## 6. Analytics
- Page view counter per post (increment on fetch, display on card + post page)
- Simple admin dashboard stats (total views, top posts, views over time)
